...
rpmbuild -bs skylable-sx.spec
#sudo yum-builddep $HOME/packages/rpmbuild/SRPMS/skylable-sx-*.src.rpm

rpmbuild -bb skylable-sx.spec

sudo yum -y --nogpgcheck localinstall $HOME/packages/rpmbuild/RPMS/*/skylable-sx*-$VERSION-*.rpm

sudo -i sxcp -D $HOME/packages/rpmbuild/RPMS/*/*.rpm sx://indian.skylable.com/vol-packages/experimental-sx/rhel/5/
