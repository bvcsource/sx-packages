...
BuildRequires: libtool-ltdl-devel, libtool, yajl-devel, pkgconfig(libcrypto), pkgconfig(openssl), zlib-devel, perl(List::Util), perl(Time::HiRes), perl(LWP::UserAgent), perl(URI), perl(URI::Escape), perl(HTTP::Date), perl(MIME::Base64), perl(Digest::HMAC_SHA1), perl(Digest::SHA), perl(JSON), pkgconfig(nss), pkgconfig(sqlite3), curl-devel
Requires: %{name}-client%{?_isa} = %{version}-%{release}, logrotate, openssl, python
...
%changelog
* Mon May 11 2015 Skylable Dev Team <dev-team@skylable.com> - 1.1-0.1.a
- Pre-release for SX 1.1
