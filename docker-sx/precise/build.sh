...
if test "$IS_RELEASE" = "yes"; then
    # ugly hack to disable extraversion in git dir
    rm -rf $SOURCES/.git
fi
...
sxcp -D --config-dir=/root/.sx $HOME/*sx*_${FULLVER}*.deb sx://indian.skylable.com/vol-packages/experimental-sx/ubuntu/
